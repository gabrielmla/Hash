...
public class HashtableClosedAddressImpl<T> extends AbstractHashtableClosedAddress<T> {
	...
	public void insert(T element) {
		if (element != null) {
			...
			if (((LinkedList<T>) table[hashIndex]).size() > 0) {
				COLLISIONS++;
				elements++;
				((LinkedList<T>) table[hashIndex]).addFirst(element);
			} else {
				elements++;
				((LinkedList<T>) table[hashIndex]).addFirst(element);
			}
	...
	public void remove(T element) {
		if (element != null) {
			...
			if(table[hashIndex] != null && search(element) != null){
				elements--;
				((LinkedList<T>) table[hashIndex]).remove(element);
			}
		}
	}
	...
	public T search(T element) {
		if(element != null){
			...
			if(table[hashIndex] != null && ((LinkedList<T>) table[hashIndex]).contains(element)){
				return element;
			}
		}
